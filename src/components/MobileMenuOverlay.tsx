...
const Section = ({
  ...
  <div className="mb-0 text-left"> {/* Changed mb-8 to mb-0 */}
    <h3 className="text-[#9d9dad] text-base mb-4">{title}</h3>
    <div className="flex flex-wrap gap-x-6 gap-y-2 text-[#131623] text-base"> {/* Changed text-sm to text-base */}
      {children}
    </div>
  </div>
);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-16">
          ...
          <nav className="text-4xl text-[#131623] font-[300] flex flex-col space-y-8 mb-8">
            ...
            <a
              href="#about"
              className={`hover:text-gray-600 w-fit inline-block link-underline-effect transition-all duration-300 ease-out ${
                isOpen ? "opacity-100 translate-y-0 delay-400" : "opacity-0 translate-y-4"
              }`}
              onClick={onClose}
            >
              About us
          ...
          <AnimatedButton
            ...
            onClick={onClose}
            className={`mb-4 transition-all duration-300 ease-out ${ /* Changed mb-8 to mb-4 */
              isOpen ? "opacity-100 translate-y-0 delay-600" : "opacity-0 translate-y-4"
            }`}
          />
        </div>
        ...
        <div>
          <div
            className={`transition-all duration-500 ease-out ${ /* Changed duration-300 to duration-500 */
              isOpen ? "opacity-100 translate-y-0 w-full delay-700" : "opacity-0 translate-y-4 w-0"
            }`}
          >
            <hr className="border-t border-gray-300 my-4" />
          </div>
          <Section> {/* Removed title="Social" */}
            <a
              href="#facebook"
              className={`inline-flex items-center link-underline-effect px-2 py-1 group relative overflow-hidden transition-all duration-300 ease-out ${
                isOpen ? "opacity-100 translate-y-0 delay-800" : "opacity-0 translate-y-4"
              }`}
            >
              <span>Facebook</span>
              <span className="relative w-6 h-6 ml-1 inline-flex items-center justify-center"> {/* Wrapper for arrows */}
                {/* Current arrow, slides out top-right */}
                <img src={ArrowIcon} alt="Arrow" className="absolute w-4 h-4 rotate-[-45deg] transition-all duration-700 ease-out group-hover:translate-x-2 group-hover:-translate-y-2 group-hover:opacity-0" />
                {/* New arrow, slides in bottom-left */}
                <img src={ArrowIcon} alt="Arrow" className="absolute w-4 h-4 rotate-[-45deg] translate-x-[-8px] translate-y-[8px] opacity-0 transition-all duration-700 ease-out group-hover:translate-x-0 group-hover:translate-y-0 group-hover:opacity-100" />
              </span>
            </a>
            <a
              href="#linkedin"
              className={`inline-flex items-center link-underline-effect px-2 py-1 group relative overflow-hidden transition-all duration-300 ease-out ${
                isOpen ? "opacity-100 translate-y-0 delay-900" : "opacity-0 translate-y-4"
              }`}
            >
              <span>LinkedIn</span>
              <span className="relative w-6 h-6 ml-1 inline-flex items-center justify-center"> {/* Wrapper for arrows */}
            ...
            <a
              href="#email"
              className={`inline-flex items-center link-underline-effect px-2 py-1 group relative overflow-hidden transition-all duration-300 ease-out ${
                isOpen ? "opacity-100 translate-y-0 delay-1000" : "opacity-0 translate-y-4"
              }`}
            >
              <span>Email</span>
              <span className="relative w-6 h-6 ml-1 inline-flex items-center justify-center"> {/* Wrapper for arrows */}
