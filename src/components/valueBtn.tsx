...
const ValueBtn: React.FC<ValueBtnProps> = ({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative overflow-hidden inline-flex items-center justify-center px-6 py-2 w-fit group cursor-pointer rounded-full border ${borderColor} transition-all duration-500 ease-out ${
        isActive ? `${activeBgColor} ${activeTextColor}` : `${baseBgColor} ${baseTextColor}`
      }`}
    >
      {/* Sliding background for hover (only if not active) */}
      {!isActive && (
        <span
          className={`absolute inset-0 z-1 block h-full w-full rounded-[48px] ${hoverBgColor} transform translate-y-[-101%] group-hover:translate-y-0 group-hover:rounded-[0] transition-all duration-500 [transition-timing-function:cubic-bezier(0.4,0,0,1)]`}
        ></span>
      )}
