...
const PROJECTS: { [key: string]: Project } = {
  ...
  "visuel-atelier": {
    ...
    tagsByCategory: {
      "Tech Stack": ["Three.js", "GSAP", "Vite", "React"],
    },
  },
  "bang-olufsen": {
    title: "Bang & Olufsen",
...
const ProjectPage: React.FC = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.from(headingRef.current, { y: 150, opacity: 0, delay: 0.4 })
        .from(descRef.current, { y: 50, opacity: 0 }, "-=1.1")
        .from(scrollPromptRef.current, { y: 30, opacity: 0 }, "-=1")
        .from(
          imageRef.current,
          { y: 100, opacity: 0, scale: 1.05, duration: 1.6 },
          "-=1.2",
        );
  ...
  return (
    ...
    >
      <div className="px-4 md:px-10 lg:px-4 xl:px-6">
        ...
        <div className="w-full flex flex-col lg:flex-row gap-16 justify-between items-start mb-32">
          <div className="text-left lg:w-[30%]">
            ...
            <p className="text-2xl md:text-2xl text-[var(--foreground)] text-left max-w-sm leading-tight opacity-60">
      ...
      <section className="px-4 md:px-10 lg:px-4 xl:px-6 pb-32 mt-32">
            ...
            {otherProjects.map((p, index) => (
              ...
              >
                ...
                <Link
                  ...
                  className={`w-full bg-[#dbdbdb] rounded-sm overflow-hidden group/thumb relative transition-all duration-700 ease-in-out cursor-pointer ${activeHoveredIndex === index + 1 ? (index === 0 ? "h-[800px]" : "h-[750px]") : index === 0 ? "h-[600px]" : "h-[550px]"}`}
                >
                  <img
                    src={p.image}
                    alt={p.title}
